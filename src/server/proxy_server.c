...
#define SERVER_IP "52.87.161.68" // Dirección IP del servidor proxy para uso en encabezados HTTP.
...
Server servers[SERVER_COUNT] = {
    {"page1.com", "/"}, // Primer servidor Apache configurado.
    {"page2.com", "/"}, // Segundo servidor Apache configurado.
    {"page3.com", "/"}  // Tercer servidor Apache configurado.
};
...
void *handle_request(void *client_socket_ptr) {
    ...
    */
    ...
    char method[10], url[MAX], protocol[10], hostname[MAX], path[MAX];
    ...
    *protocol_end = '\0'; // Termina la cadena del protocolo.
    ...
    for (int i = 0; i < SERVER_COUNT; i++) {
        if (strcmp(hostname, servers[i].hostname) == 0) {
            printf("\n\nDesde adentro\n\n");
    ...
    if (obtener_respuesta_cache(cache_file_name, respuesta_cache, ttl_global)) {
        ...
        log_message("Respuesta enviada a %s, con encabezado Via: ubuntu@%s", hostname, SERVER_IP);
